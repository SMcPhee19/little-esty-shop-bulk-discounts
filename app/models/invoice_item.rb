class InvoiceItem < ApplicationRecord
  ...
  def discount_applies?
    ...
    quantity_thresholds = item.merchant.bulk_discounts # item.merchant.bulk_discounts is querying the
    ...
    quantity_thresholds.any? { |q| quantity >= q } # q's the value given to each element of the array of quantity values
  ...
  def discount_applied
    bulk_discounts.where('? >= bulk_discounts.quantity', self.quantity) #  querying the bulk_discounts table to find any
                  # records where the quanity is less than or equal to the quantity of the invoice item
                  .order(percent: :desc).take # .take is taking the first record from the ordered results/
